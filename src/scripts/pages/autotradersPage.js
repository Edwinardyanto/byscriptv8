...
const renderAutotradersTable = ({ rows, page, pageSize }) => {
  ...
  for (const r of pageRows) {
    ...
    tr.innerHTML = `
      ...
      <td><span class="autotrader-name">${r.autotrader_name}</span></td>
      <td class="col-trading-plan"><span class="trading-plan-name">${r.trading_plan_name}</span></td>
      <td>${r.pair}</td>
      <td>${r.provider_label}</td>
      ...
      <td class="numeric">${r.running_count}</td>
...
const initAutotradersPage = async () => {
  ...
  const netQtyByAutotrader = new Map();
  ...
  for (const t of tradesAll) {
    const id = t && t.autotrader_id;
    if (!id) continue;
    const side = String(t.side || "").toLowerCase();
    const qty = safeNum(t.size);
    const signed = side === "sell" ? -qty : qty;
    netQtyByAutotrader.set(id, (netQtyByAutotrader.get(id) || 0) + signed);
  }
}
  ...
  const state = {
    ...
    rows: (autotraders || []).map((a) => {
      ...
      const providerKey = account?.exchange || account?.provider || "exchange";
      const providerLabel = toTitle(providerKey);

      const netQty = safeNum(netQtyByAutotrader.get(a.autotrader_id) || 0);
      const runningCount = Math.abs(netQty) > 1e-9 ? 1 : 0;

      return {
        autotrader_id: a.autotrader_id,
        status: a.status,
        autotrader_name: a.autotrader_name || "Autotrader",
        trading_plan_name: plan?.name || "Trading Plan",
        pair,
