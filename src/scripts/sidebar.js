...
export const initSidebar = (appRoot) => {
  ...
  if (!appRoot.querySelector(".sidebar-toggle")) {
    ...
    // Prefer mounting the toggle inside the standardized global header.
    // Fallback: prepend to appRoot (older layouts).
    const headerLeftSlot = appRoot.querySelector("[data-header-left]");
    if (headerLeftSlot) {
      headerLeftSlot.appendChild(toggle);
    } else {
      appRoot.prepend(toggle);
    }
  }
